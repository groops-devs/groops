...
It is recommended to perform the steps above in a \reference{loop}{general.loopsAndConditions} over all satellites/PRNs
using \program{LoopPrograms}. Since \program{PreprocessingVariationalEquation} requires an
\configFile{inputfileSatelliteModel}{satelliteModel}, which is generally defined for a specific satellite/SVN or block,
\program{GnssPrn2SvnBlockVariables} can be used before \program{PreprocessingVariationalEquation} to define
\reference{variables}{general.parser} for SVN and block based on the PRN.
These variables are then available in \configFile{inputfileSatelliteModel}{satelliteModel}.

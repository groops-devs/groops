/***********************************************/
/**
* @file gnssParametrizationTransmitterGalileo.cpp
*
...
GnssParametrizationTransmitterGalileo::GnssParametrizationTransmitterGalileo(Config &config)
{
  try
  {
    ...
    readConfig(config, "inputfileTransmitterList",      fileNameTransmitterList,       Config::MUSTSET,  "{groopsDataDir}/gnss/transmitterGalileo/transmitterList.galileo.txt", "ascii file with transmitter PRNs, used to loop variable {prn}");
    readConfig(config, "inputfileTransmitterInfo",      fileNameTransmitterInfo,       Config::MUSTSET,  "{groopsDataDir}/gnss/transmitterGalileo/transmitterInfo/igs/igs14/transmitterInfo_igs14.{prn}.xml", "variable {prn} available");
    readConfig(config, "inputfileAntennaDefintion",     fileNameAntennaDef,            Config::MUSTSET,  "{groopsDataDir}/gnss/transmitterGalileo/antennaDefinition/igs/igs14/antennaDefinition_igs14.xml", "phase centers and variations (ANTEX)");
    readConfig(config, "inputfileReceiverDefintion",    fileNameReceiverDef,           Config::OPTIONAL, "{groopsDataDir}/gnss/transmitterGalileo/receiverDefinition/receiverDefinition.xml", "transmitted signal types");
    ...
    if(readConfigChoice(config, "estimateClockError", choice, Config::OPTIONAL, "", ""))
    {
      if(readConfigChoiceElement(config, "epochWise", choice, "clock error is estimated for each epoch"))
      {
        ...
        readConfig(config, "sigmaZeroMeanConstraint", sigmaClockZeroMean, Config::DEFAULT,  "0", "(0 = unconstrained) sigma [m] for zero-mean constraint over all satellite clocks");
      }

...
public:
  ExpressionValue(Double v) : value(v) {}
  std::string   string() const;
  void          usedVariables(const VariableList &/*varList*/, std::set<std::string> &/*usedName*/) const {}
  ExpressionPtr simplify(const VariableList &/*varList*/, Bool &resolved) const {resolved = TRUE; return clone();}
  Double        evaluate(const VariableList &/*varList*/) const {return value;}
  ExpressionPtr derivative(const std::string &/*var*/) const {return std::make_shared<ExpressionValue>(0);}
  ExpressionPtr clone() const {return std::make_shared<ExpressionValue>(value);}
  UInt          priority() const {return Expression::Priority::VALUE;}
};
...
public:
  ExpressionVar(const std::string &_name) : name(_name) {}
  std::string   string() const {return name;}
  void          usedVariables(const VariableList &varList, std::set<std::string> &usedName) const;
  ExpressionPtr simplify(const VariableList &varList, Bool &resolved) const;
  Double        evaluate(const VariableList &varList) const;
  ExpressionPtr derivative(const std::string &var) const  {return exprValue((var == this->name) ? 1 : 0);}
  ExpressionPtr clone() const {return std::make_shared<ExpressionVar>(name);}
  UInt          priority() const {return Expression::Priority::VALUE;}
};
...
public:
  ...
  ExpressionPtr simplify(const VariableList &/*varList*/, Bool &resolved) const {resolved = TRUE; return clone();}
  Double        evaluate(const VariableList &/*varList*/) const {return value;}
  ExpressionPtr derivative(const std::string &/*var*/) const {return exprValue(0);}
  ExpressionPtr clone()  const {return create();}
  UInt          priority() const {return Expression::Priority::FUNCTION;}
};
...
public:
  ...
  virtual std::string   string() const {return name()+"("+operand->string()+")";}
  virtual ExpressionPtr create(const ExpressionPtr &ob) const = 0;
  virtual void          usedVariables(const VariableList &varList, std::set<std::string> &usedName) const {operand->usedVariables(varList, usedName);}
  virtual ExpressionPtr simplify(const VariableList &varList, Bool &resolved) const;
  virtual ExpressionPtr derivative(const std::string &/*var*/) const {throw(Exception("Derivative not defined for \""+name()+"\"."));}
  virtual ExpressionPtr clone()  const {return create(operand->clone());}
  virtual UInt          priority() const {return Expression::Priority::FUNCTION;}
};
...
public:
  ...
  virtual std::string   name()   const {return "operator";}
  virtual std::string   string() const {return name()+"("+left->string()+", "+right->string()+")";}
  virtual ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const = 0;
  virtual void          usedVariables(const VariableList &varList, std::set<std::string> &usedName) const {left->usedVariables(varList, usedName); right->usedVariables(varList, usedName);}
  virtual ExpressionPtr simplify(const VariableList &varList, Bool &resolved) const;
  virtual ExpressionPtr derivative(const std::string &/*var*/) const {throw(Exception("Derivative not defined for \""+name()+"\"."));}
  virtual ExpressionPtr clone()  const {return create(left->clone(), right->clone());}
  virtual UInt priority() const {return Expression::Priority::FUNCTION;}
};
...
public:
  ...
  virtual std::string   name()   const {return "operator";}
  virtual std::string   string() const {return name()+"("+arg1->string()+", "+arg2->string()+", "+arg3->string()+")";}
  virtual ExpressionPtr create(const ExpressionPtr &a1, const ExpressionPtr &a2, const ExpressionPtr &a3) const = 0;
  virtual void          usedVariables(const VariableList &varList, std::set<std::string> &usedName) const {arg1->usedVariables(varList, usedName); arg2->usedVariables(varList, usedName); arg3->usedVariables(varList, usedName);}
  virtual ExpressionPtr simplify(const VariableList &varList, Bool &resolved) const;
  virtual ExpressionPtr derivative(const std::string &/*var*/) const {throw(Exception("Derivative not defined for \""+name()+"\"."));}
  virtual ExpressionPtr clone()  const {return create(arg1->clone(), arg2->clone(), arg3->clone());}
  virtual UInt priority() const {return Expression::Priority::FUNCTION;}
};
...
public:
  ...
  std::string   name()   const {return "pi";}
  ExpressionPtr create() const {return std::make_shared<ExpressionPi>();}
};
...
public:
  ...
  std::string   name()   const {return "rho";}
  ExpressionPtr create() const {return std::make_shared<ExpressionRho>();}
};
...
public:
  ...
  std::string   name()   const {return "nan";}
  ExpressionPtr create() const {return std::make_shared<ExpressionNan>();}
};
...
{
public:
  ExpressionNegative(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   string() const {return "-"+operand->string(priority());}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionNegative>(ob);}
  Double        evaluate(const VariableList &v) const {return -operand->evaluate(v);}
  ExpressionPtr derivative(const std::string &var) const {return -operand->derivative(var);}
  UInt priority() const {return Expression::Priority::UNARY;}
};
...
public:
  ...
  std::string   string() const {return left->string(priority()) + " + " + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionAdd>(l, r);}
  Double        evaluate(const VariableList &v) const {return left->evaluate(v) + right->evaluate(v);}
  ExpressionPtr derivative(const std::string &var) const {return left->derivative(var) + right->derivative(var);}
  UInt priority() const {return Expression::Priority::ADDITIVE;}
};
...
public:
  ...
  std::string   string() const {return left->string(priority()) + " - " + right->string(priority()+1);}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionSub>(l, r);}
  Double        evaluate(const VariableList &v) const {return left->evaluate(v) - right->evaluate(v);}
  ExpressionPtr derivative(const std::string &var) const {return left->derivative(var) - right->derivative(var);}
  UInt priority() const {return Expression::Priority::ADDITIVE;}
};
...
public:
  ...
  std::string   string() const {return left->string(priority()) + "*" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionMult>(l, r);}
  Double        evaluate(const VariableList &v) const {return left->evaluate(v) * right->evaluate(v);}
  ExpressionPtr derivative(const std::string &var) const {return left->derivative(var)*right->clone() + left->clone()*right->derivative(var);}
  UInt priority() const {return Expression::Priority::MULTIPLICATIVE;}
};
...
public:
  ...
  std::string string() const {return left->string(priority()) + "/" + right->string(priority()+1);}
  inline ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionDiv>(l, r);}
  inline Double        evaluate(const VariableList &v) const {return left->evaluate(v) / right->evaluate(v);}
  inline ExpressionPtr derivative(const std::string &var) const {return (left->derivative(var)*right->clone() - left->clone()*right->derivative(var))/(right->clone()^exprValue(2));}
  UInt priority() const {return Expression::Priority::MULTIPLICATIVE;}
};
...
public:
  ...
  std::string   string() const {return left->string(priority()) + "^" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionPow>(l, r);}
  Double        evaluate(const VariableList &v) const {return pow(left->evaluate(v),right->evaluate(v));}
  ExpressionPtr derivative(const std::string &var) const {return left->derivative(var)*right->clone()/left->clone() * (left->clone()^(right->clone()-exprValue(1)));} // this derivative is not completly correct!!!!
  UInt priority() const {return Expression::Priority::EXPONENTIAL;}
};
...
public:
  ...
  std::string   name()   const {return "operator<";}
  std::string   string() const {return left->string(priority()) + "<" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionLessThan>(l, r);}
  Double        evaluate(const VariableList &varList) const {return left->evaluate(varList) < right->evaluate(varList) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::RELATION;}
};
...
public:
  ...
  std::string   name()   const {return "operator<=";}
  std::string   string() const {return left->string(priority()) + "<=" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionLessEqualThan>(l, r);}
  Double        evaluate(const VariableList &varList) const {return left->evaluate(varList) <= right->evaluate(varList) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::RELATION;}
};
...
public:
  ...
  std::string   name()   const {return "operator>";}
  std::string   string() const {return left->string(priority()) + ">" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionGreaterThan>(l, r);}
  Double        evaluate(const VariableList &varList) const {return left->evaluate(varList) > right->evaluate(varList) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::RELATION;}
};
...
public:
  ...
  std::string   name()   const {return "operator>=";}
  std::string   string() const {return left->string(priority()) + ">=" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionGreaterEqualThan>(l, r);}
  Double        evaluate(const VariableList &varList) const {return left->evaluate(varList) >= right->evaluate(varList) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::RELATION;}
};
...
public:
  ...
  std::string   name()   const {return "operator==";}
  std::string   string() const {return left->string(priority()) + "==" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionEqual>(l, r);}
  Double        evaluate(const VariableList &varList) const {return left->evaluate(varList) == right->evaluate(varList) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::EQUALITY;}
};
...
public:
  ...
  std::string   name()   const {return "operator!=";}
  std::string   string() const {return left->string(priority()) + "!=" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionNotEqual>(l, r);}
  Double        evaluate(const VariableList &varList) const {return left->evaluate(varList) != right->evaluate(varList) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::EQUALITY;}
};
...
{
public:
  ExpressionLogicalNot(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name()   const {return "operator!";}
  std::string   string() const {return "!"+operand->string(priority());}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionLogicalNot>(ob);}
  Double        evaluate(const VariableList &v) const {return operand->evaluate(v) == 0.0 ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::UNARY;}
};
...
public:
  ...
  std::string   name()   const {return "operator&&";}
  std::string   string() const {return left->string(priority()) + "&&" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionLogicalAnd>(l, r);}
  Double        evaluate(const VariableList &varList) const {return (left->evaluate(varList)!=0.0 &&  right->evaluate(varList)!=0.0) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::LOGICAL_AND;}
};
...
public:
  ...
  std::string   name()   const {return "operator||";}
  std::string   string() const {return left->string(priority()) + "||" + right->string(priority());}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionLogicalOr>(l, r);}
  Double        evaluate(const VariableList &varList) const {return (left->evaluate(varList)!=0.0 || right->evaluate(varList)!=0.0) ? 1.0 : 0.0;}
  UInt priority() const {return Expression::Priority::LOGICAL_OR;}
};
...
{
public:
  ExpressionSqrt(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "sqrt";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionSqrt>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::sqrt(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return operand->derivative(var)/sqrt(operand->clone());}
};
...
{
public:
  ExpressionExp(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "exp";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionExp>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::exp(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return operand->derivative(var)*exp(operand->clone());}
};
...
{
public:
  ExpressionLog(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "log";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionExp>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::log(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return operand->derivative(var)^exprValue(-1);}
};
...
{
public:
  ExpressionSin(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "sin";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionSin>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::sin(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return operand->derivative(var)*cos(operand->clone());}
};
...
{
public:
  ExpressionCos(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "cos";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionCos>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::cos(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return -operand->derivative(var)*sin(operand->clone());}
};
...
{
public:
  ExpressionTan(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "tan";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionTan>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::tan(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return operand->derivative(var)/(cos(operand->clone())^exprValue(2));}
};
...
{
public:
  ExpressionAsin(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "asin";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionAsin>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::asin(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return operand->derivative(var)/sqrt(exprValue(1) - ((operand->clone())^exprValue(2)));}
};
...
{
public:
  ExpressionAcos(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "acos";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionAcos>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::acos(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return -operand->derivative(var)/sqrt(exprValue(1) - ((operand->clone())^exprValue(2)));}
};
...
{
public:
  ExpressionAtan(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "atan";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionAtan>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::atan(operand->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return operand->derivative(var)/(exprValue(1) + ((operand->clone())^exprValue(2)));}
};
...
{
public:
  ExpressionAbs(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "abs";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionAbs>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::fabs(operand->evaluate(varList));}
};
...
{
public:
  ExpressionRound(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "round";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionRound>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::round(operand->evaluate(varList));}
};
...
{
public:
  ExpressionCeil(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "ceil";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionCeil>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::ceil(operand->evaluate(varList));}
};
...
{
public:
  ExpressionFloor(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "floor";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionFloor>(ob);}
  Double        evaluate(const VariableList &varList) const {return std::floor(operand->evaluate(varList));}
};
...
{
public:
  ExpressionGPS2UTC(const ExpressionPtr &ob) : ExpressionFunction1(ob) {}
  std::string   name() const {return "gps2utc";}
  ExpressionPtr create(const ExpressionPtr &ob) const {return std::make_shared<ExpressionFloor>(ob);}
  Double        evaluate(const VariableList &varList) const {return timeGPS2UTC(mjd2time(operand->evaluate(varList))).mjd();}
};
...
public:
  ...
  std::string   name() const {return "atan2";}
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionAtan2>(l, r);}
  Double        evaluate(const VariableList &varList) const {return std::atan2(left->evaluate(varList), right->evaluate(varList));}
  ExpressionPtr derivative(const std::string &var) const {return (left->derivative(var)*right->clone() - left->clone()*right->derivative(var))/((left->clone()^exprValue(2))+(right->clone()^exprValue(2)));}
};
...
public:
  ...
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionMin>(l, r);}
  Double        evaluate(const VariableList &varList) const {return std::min(left->evaluate(varList), right->evaluate(varList));}
};
...
public:
  ...
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionMax>(l, r);}
  Double        evaluate(const VariableList &varList) const {return std::max(left->evaluate(varList), right->evaluate(varList));}
};
...
public:
  ...
  ExpressionPtr create(const ExpressionPtr &l, const ExpressionPtr &r) const {return std::make_shared<ExpressionMod>(l, r);}
  Double        evaluate(const VariableList &varList) const {return std::fmod(left->evaluate(varList), right->evaluate(varList));}
};
...
public:
  ...
  ExpressionPtr create(const ExpressionPtr &a1, const ExpressionPtr &a2, const ExpressionPtr &a3) const {return std::make_shared<ExpressionIfThenElse>(a1, a2, a3);}
  Double        evaluate(const VariableList &varList) const {return arg1->evaluate(varList) != 0.0 ? arg2->evaluate(varList) : arg3->evaluate(varList);}
};
...
{
public:
  ExpressionIsNan(const ExpressionPtr &a1) : ExpressionFunction1(a1) {}
  std::string   name() const {return "isnan";}
  ExpressionPtr create(const ExpressionPtr &a1) const {return std::make_shared<ExpressionIsNan>(a1);}
  Double        evaluate(const VariableList &varList) const {return std::isnan(operand->evaluate(varList));}
};
...
public:
  ...
  std::string   name() const {return "date2mjd";}
  ExpressionPtr create(const ExpressionPtr &a1, const ExpressionPtr &a2, const ExpressionPtr &a3) const {return std::make_shared<ExpressionDate2mjd>(a1, a2, a3);}
  Double        evaluate(const VariableList &varList) const {return date2time(static_cast<UInt>(arg1->evaluate(varList)), static_cast<UInt>(arg2->evaluate(varList)), static_cast<UInt>(arg3->evaluate(varList))).mjd();}
};
...
{
public:
  ExpressionDayOfYear(const ExpressionPtr &a1) : ExpressionFunction1(a1) {}
  std::string   name() const {return "dayofyear";}
  ExpressionPtr create(const ExpressionPtr &a1) const {return std::make_shared<ExpressionDayOfYear>(a1);}
  Double        evaluate(const VariableList &varList) const {return static_cast<Double>(mjd2time(operand->evaluate(varList)).dayOfYear());}
};
...
{
public:
  ExpressionDecimalYear(const ExpressionPtr &a1) : ExpressionFunction1(a1) {}
  std::string   name() const {return "decimalyear";}
  ExpressionPtr create(const ExpressionPtr &a1) const {return std::make_shared<ExpressionDecimalYear>(a1);}
  Double        evaluate(const VariableList &varList) const {return mjd2time(operand->evaluate(varList)).decimalYear();}
};
...
ExpressionPtr operand(Tokenizer &token)
{
  ...
  {
    std::string nameLowerCase = token.name;
    ...
    for(UInt i=0; i<FunctionList::func2List.size(); i++)
      if(nameLowerCase == FunctionList::func2List.at(i)->name())
      {
        token.get();
        if((token.type!=Tokenizer::SINGLE)||(token.c!='('))
          throw(Exception(token.infoString("expected '('")));
    ...
    for(UInt i=0; i<FunctionList::func3List.size(); i++)
      if(nameLowerCase == FunctionList::func3List.at(i)->name())
      {
        token.get();
        if((token.type!=Tokenizer::SINGLE)||(token.c!='('))
          throw(Exception(token.infoString("expected '('")));

  ...
  try
  {
    ...
    if(!fileNameSinex.empty())
    {
      ...
      for(std::string &line : sinex.findBlock("SITE/ID")->lines)
      {
        ...
        const Double latitude    = String::toDouble(line.substr(56, 3)) + (String::toDouble(line.substr(56, 3)) < 0 ? -1 : 1) * (String::toDouble(line.substr(60, 2))/60 + String::toDouble(line.substr(63, 4))/3600);
